...
void GraphColoring(std::vector<std::vector<double>>& Q, int size,int num_colors, std::vector<std::pair<std::vector<int>,int>>& nhot_memo){
    ...
    for(int i=0;i<size;++i){
        for(int j=0;j<num_colors;++j){
            if(i==0){
                ...
                Q[i*num_colors+j][(i+size)*num_colors+j]+=c;
            }
            else if(i==size-1){
                Q[i*num_colors+j][(i+size)*num_colors+j]+=c;
                Q[i*num_colors+j][(i-1)*num_colors+j]+=c;
...
int main(){
    ...
    GraphColoring(Q,size,colors,nhot_memo);
